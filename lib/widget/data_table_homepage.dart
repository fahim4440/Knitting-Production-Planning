...
import 'package:knitting_production_planning/widget/head_cell_container.dart';
import '../bloc/homepage_bloc/homepage_bloc.dart';
import 'cell_container.dart';

Widget DataTableExcelHomepage(HomepageLoadedState state) {
  final List<DataColumn> headerColumn = [
    ...
    DataColumn(label: HeadCellContainer("Dia"), numeric: true,),
    DataColumn(label: HeadCellContainer("Gauge"), numeric: true,),
  ...
  return state.orders.isNotEmpty ? DataTable(
    ...
    rows: state.orders.map((order) {
      return DataRow(cells: [
        ...
        order.runningMachine == null ? const DataCell(Text("")) : DataCell(CellContainer(order.runningMachine!.length.toString())),
